...
type RestOf<T extends IEmployee | NewEmployee> = Omit<
  ...
  | 'dateResigned'
> & {
  nicIssueDate?: string | null;
  passportIssueDate?: string | null;
  passportExpDate?: string | null;
  dateOfBirth?: string | null;
  marriedDate?: string | null;
  empRegDate?: string | null;
  lmd?: string | null;
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      nicIssueDate: employee.nicIssueDate?.format(DATE_FORMAT) ?? null,
      passportIssueDate: employee.passportIssueDate?.format(DATE_FORMAT) ?? null,
      passportExpDate: employee.passportExpDate?.format(DATE_FORMAT) ?? null,
      dateOfBirth: employee.dateOfBirth?.format(DATE_FORMAT) ?? null,
      marriedDate: employee.marriedDate?.format(DATE_FORMAT) ?? null,
      empRegDate: employee.empRegDate?.format(DATE_FORMAT) ?? null,
      lmd: employee.lmd?.toJSON() ?? null,
      dateJoined: employee.dateJoined?.toJSON() ?? null,
      dateResigned: employee.dateResigned?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      nicIssueDate: restEmployee.nicIssueDate ? dayjs(restEmployee.nicIssueDate) : undefined,
      passportIssueDate: restEmployee.passportIssueDate ? dayjs(restEmployee.passportIssueDate) : undefined,
      passportExpDate: restEmployee.passportExpDate ? dayjs(restEmployee.passportExpDate) : undefined,
      dateOfBirth: restEmployee.dateOfBirth ? dayjs(restEmployee.dateOfBirth) : undefined,
      marriedDate: restEmployee.marriedDate ? dayjs(restEmployee.marriedDate) : undefined,
      empRegDate: restEmployee.empRegDate ? dayjs(restEmployee.empRegDate) : undefined,
      lmd: restEmployee.lmd ? dayjs(restEmployee.lmd) : undefined,
      dateJoined: restEmployee.dateJoined ? dayjs(restEmployee.dateJoined) : undefined,
      dateResigned: restEmployee.dateResigned ? dayjs(restEmployee.dateResigned) : undefined,
    };
  }
