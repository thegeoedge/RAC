...
import { EntityArrayResponseType, TransactionsService } from '../service/transactions.service';
...
export class TransactionsComponent implements OnInit {
  ...
  transactions = signal<ITransactions[]>([]);
  isLoading = false;
  ...
  trackId = (item: ITransactions): number => this.transactionsService.getTransactionsIdentifier(item);
  ...
  protected refineData(data: ITransactions[]): ITransactions[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: ITransactions[] | null): ITransactions[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.transactionsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
