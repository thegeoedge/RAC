...
type RestOf<T extends ITransactions | NewTransactions> = Omit<T, 'date' | 'lmd'> & {
  date?: string | null;
  lmd?: string | null;
};
...
export class TransactionsService {
  ...
  create(transactions: any): Observable<EntityResponseType> {
  ...
  addTransactionsToCollectionIfMissing<Type extends Pick<ITransactions, 'id'>>(
    ...
    if (transactions.length > 0) {
      const transactionsCollectionIdentifiers = transactionsCollection.map(transactionsItem =>
        this.getTransactionsIdentifier(transactionsItem),
      );
  ...
  protected convertDateFromClient<T extends ITransactions | NewTransactions | PartialUpdateTransactions>(transactions: T): RestOf<T> {
    return {
      ...transactions,
      date: transactions.date?.toJSON() ?? null,
      lmd: transactions.lmd?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransactions: RestTransactions): ITransactions {
    return {
      ...restTransactions,
      date: restTransactions.date ? dayjs(restTransactions.date) : undefined,
      lmd: restTransactions.lmd ? dayjs(restTransactions.lmd) : undefined,
    };
  }
